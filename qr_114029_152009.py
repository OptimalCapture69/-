...
cv2.createTrackbar('min_height', 'size', 0, 480, nothing)
cv2.createTrackbar('min_width', 'size', 0, 640, nothing)

cv2.createTrackbar('max_height', 'size', 0, 480, nothing)
cv2.createTrackbar('max_width', 'size', 0, 640, nothing)
	...
	for cnt in contours:
		...
		if Areab > noise:
			...
			if min_width <= w1 <= max_width and min_height <= h1 <= max_height:
				cv2.rectangle (img, (x1, y1), (X2, Y2), (0qq,255,0), 2)
				print(w1, h1, min_width, max_width, min_height, max_height)
			print(sm*w1) #a
